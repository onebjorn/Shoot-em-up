...
public:
  ...
  Obstacle2D(float const x, float const y)
    : Box2D (x - ObstacleSizeX / 2, y - ObstacleSizeY / 2 , x + ObstacleSizeX / 2, y + ObstacleSizeY / 2)
    , m_obstacleHealth(ObstacleHealth)
  {}

  Obstacle2D(float const x1, float const y1, float const x2, float const y2)
    : Box2D (x1, y1, x2, y2)
    , m_obstacleHealth(ObstacleHealth)
  {}

  Obstacle2D(Point2D const & leftBottom, Point2D const & rightTop, float health)
    : Box2D(leftBottom, rightTop)
    , m_obstacleHealth(health)
  {}

  Obstacle2D(Point2D const & center)
    : Box2D(center.x() - ObstacleSizeX / 2, center.y() - ObstacleSizeY / 2, center.x() + ObstacleSizeX / 2, center.y() + ObstacleSizeY / 2)
    , m_obstacleHealth(ObstacleHealth)
  {}

  Obstacle2D(Point2D const & leftBottom, Point2D const & rightTop)
    : Box2D(leftBottom, rightTop)
    , m_obstacleHealth(ObstacleHealth)
  {}

  Obstacle2D( Box2D const & obstacleBox)
